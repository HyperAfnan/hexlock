    ...
    const checkAuthentication = async () => {
      try {
        ...
        if (isLoggedIn) {
          ...
          console.log("Found cached principal ID, but user is not authenticated");
          ...
          try {
            ...
            console.error("Error authenticating with cached principal ID:", error);
          ...
          <div className="flex items-center space-x-4">
            {principalId && (
              <span className={`hidden md:block text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                ID: {principalId.substring(0, 5)}...{principalId.substring(principalId.length - 5)}
              </span>
            )}
            <button
...
const LandingContent = ({ isDarkMode }: { isDarkMode: boolean }) => {
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div className={`min-h-screen pt-16 ${isDarkMode ? 'bg-[#1a1f2e]' : 'bg-gray-50'}`}>
            ...
            <div className="reveal">
              <img
                ...
                onError={(e) => {
                  ...
                  target.src = "https://via.placeholder.com/800x600?text=Security+Dashboard";
                }}
              />
            </div>
          </div>
